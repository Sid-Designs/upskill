    ...
    <AuthLayout>
      <div className="w-full h-full flex flex-col justify-center px-5 py-8 sm:px-8 sm:py-10 gap-8">
        <div className="text-center space-y-2">
          <p className="text-xl sm:text-2xl font-semibold text-[var(--color-primary)]">Welcome back</p>
          <h1 className="text-2xl sm:text-3xl font-semibold">Sign in to continue</h1>
          <p className="text-xs sm:text-sm text-[var(--color-muted)]">Stay synced across devices and keep your applications moving.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mx-auto flex w-full max-w-xl flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between text-sm text-[var(--color-muted)]">
            <label className="inline-flex items-center gap-2 cursor-pointer select-none">
              <input type="checkbox" className="h-4 w-4 rounded border-[var(--color-border)]" defaultChecked disabled={isSubmitting} />
              <span>Remember me</span>
            </label>
            <Link href="/auth/forgot-password" className="hover:text-[var(--color-primary)] transition-colors">
              Forgot password?
            </Link>
          </div>
